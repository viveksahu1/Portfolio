import { ref, unref, useId, onMounted, onBeforeUnmount, onUpdated, isRef, computed, defineComponent, openBlock, createBlock, mergeProps, withCtx, renderSlot, toRefs, resolveDynamicComponent, TransitionGroup, Transition, reactive } from 'vue';
import { createContext, getElFromTemplateRef, Primitive } from '@oku-ui/primitives';
export * from 'motion';
...
const svgElementNames = [
  ...
  // Draws a circle.
  "ellipse",
  // Draws an ellipse, commonly used for ovals.
  "line",
  // Draws a straight line between two points.
  "polygon",
  // Creates a closed shape with multiple vertices.
  "polyline",
  // Creates a series of connected line segments.
  "rect",
  // Draws a rectangle with optional rounded corners.
  "path",
  // Defines a custom shape or path, supporting complex curves.
  "text",
  // Displays text within the SVG.
  "tspan",
  // Sub-element of <text>, used to style or position portions of text.
  "textPath",
  // Aligns text along a defined path.
  // Grouping and definitions
  "g",
  // Groups multiple SVG elements together.
  "defs",
  // Container for reusable definitions (e.g., gradients or symbols).
  "symbol",
  // Defines a reusable graphic, typically referenced with <use>.
  "use",
  // References and reuses a defined element or graphic.
  "desc",
  // Provides a description for accessibility or documentation purposes.
  "title",
  // Adds a title for accessibility or tooltips.
  // Filters and effects
  "filter",
  // Applies visual effects like blurring or lighting.
  "feBlend",
  // Blends multiple graphics together.
  "feColorMatrix",
  // Applies color transformations using a matrix.
  "feComponentTransfer",
  // Alters individual RGBA channels.
  "feComposite",
  // Combines images or graphics.
  "feConvolveMatrix",
  // Applies a custom kernel-based filter.
  "feDiffuseLighting",
  // Simulates diffuse lighting effects.
  "feDisplacementMap",
  // Warps an image or graphic using a displacement map.
  "feDropShadow",
  // Adds drop shadow effects.
  "feFlood",
  // Fills the canvas or graphic with a solid color.
  "feGaussianBlur",
  // Blurs the graphic using a Gaussian function.
  "feImage",
  // Renders an external image in an SVG context.
  "feMerge",
  // Combines multiple filter results.
  "feMorphology",
  // Erodes or dilates the edges of a shape.
  "feOffset",
  // Shifts graphics by a specified distance.
  "feSpecularLighting",
  // Simulates shiny lighting effects.
  "feTile",
  // Repeats graphics in a tiled pattern.
  "feTurbulence",
  // Adds texture or noise for a natural effect.
  // Animation
  "animate",
  // Animates a property over time.
  "animateMotion",
  // Moves an element along a defined path.
  "animateTransform",
  // Animates transformation properties like scale or rotate.
  "set",
  // Sets a property for a specific duration.
  // Masks and clipping paths
  "mask",
  // Creates a mask to hide or reveal portions of graphics.
  "clipPath",
  // Defines a clipping region to restrict rendering.
  ...
  "stop",
  // Specifies a color stop in a gradient.
  "pattern",
  // Defines a repeating pattern.
  // Additional elements
  "foreignObject",
  // Embeds non-SVG content (like HTML) within SVG.
  "style",
  // Embeds CSS styles for SVG elements.
  "script"
  // Embeds or links JavaScript code within SVG.
];
function isSvgElementName(tag) {
  return svgElementNames.includes(tag);
}
 ...
 */
const transformProps$3 = new Set(transformPropOrder$2);

function isForcedMotionValue(key, { layout, layoutId }) {
    return (transformProps$3.has(key) ||
...
function scrapeMotionValuesFromProps(props, prevProps, visualElement) {
    ...
    for (const key in props) {
        if (isMotionValue(props[key]) ||
            ...
            const targetKey = transformPropOrder$2.indexOf(key) !== -1
...
const degrees$1 = /*@__PURE__*/ createUnitType$1("deg");
...
const isNumOrPxType = (v) => v === number$1 || v === px$1;
...
const nonTranslationalTransformKeys = transformPropOrder$2.filter((key) => !transformKeys.has(key));
...
const hsla = {
    ...
    transform: ({ hue, saturation, lightness, alpha: alpha$1$1 = 1 }) => {
        return ("hsla(" +
            Math.round(hue) +
            ", " +
            percent$1.transform(sanitize(saturation)) +
            ", " +
            percent$1.transform(sanitize(lightness)) +
            ", " +
            sanitize(alpha$1.transform(alpha$1$1)) +
            ")");
    },
 ...
 */
const dimensionValueTypes = [number$1, px$1, percent$1, degrees$1, vw, vh, auto];
/**
...
const browserNumberValueTypes$1 = {
    // Border props
    borderWidth: px$1,
    borderTopWidth: px$1,
...
const numberValueTypes$1 = {
    ...browserNumberValueTypes$1,
...
class VisualElement {
    ...
    bindToMotionValue(key, value) {
        ...
        const valueIsTransform = transformProps$3.has(key);
...
const checkStringStartsWith$1 = (token) => (key) => typeof key === "string" && key.startsWith(token);
const isCSSVariableName$1 = 
/*@__PURE__*/ checkStringStartsWith$1("--");
const startsAsVariableToken = 
/*@__PURE__*/ checkStringStartsWith$1("var(--");
 ...
 */
const getValueAsType$1 = (value, type) => {
...
const numTransforms$1 = transformPropOrder$2.length;
/**
...
function buildTransform$1(latestValues, transform, transformTemplate) {
    ...
    for (let i = 0; i < numTransforms$1; i++) {
        const key = transformPropOrder$2[i];
...
function buildHTMLStyles$1(state, latestValues, transformTemplate) {
    ...
    if (!latestValues.transform) {
        if (hasTransform || transformTemplate) {
            style.transform = buildTransform$1(latestValues, state.transform, transformTemplate);
        }
...
class HTMLVisualElement extends DOMVisualElement {
    ...
    readValueFromInstance(instance, key) {
        if (transformProps$3.has(key)) {
            ...
            const value = (isCSSVariableName$1(key)
...
const underDampedSpring$1 = {
    type: "spring",
    stiffness: 500,
    damping: 25,
    restSpeed: 10,
};
const criticallyDampedSpring$1 = (target) => ({
    type: "spring",
    stiffness: 550,
    damping: target === 0 ? 2 * Math.sqrt(550) : 30,
    restSpeed: 10,
});
const keyframesTransition$1 = {
    type: "keyframes",
    duration: 0.8,
};
/**
...
const getDefaultTransition$1 = (valueKey, { keyframes }) => {
    if (keyframes.length > 2) {
        return keyframesTransition$1;
    }
    else if (transformProps$3.has(valueKey)) {
        return valueKey.startsWith("scale")
            ? criticallyDampedSpring$1(keyframes[1])
            : underDampedSpring$1;
    }
    return ease$1;
};
...
function animateTarget(visualElement, targetAndTransition, { delay = 0, transitionOverride, type } = {}) {
    ...
    for (const key in target) {
        ...
        value.start(animateMotionValue(key, value, valueTarget, visualElement.shouldReduceMotion && transformProps$3.has(key)
            ? { type: false }
...
function animateSubject(subject, keyframes, options, scope) {
    ...
    if (isSingleValue(subject, keyframes)) {
        ...
        const numSubjects = subjects.length;
        invariant$1(Boolean(numSubjects), "No valid elements provided.");
...
function createStyles(keyframes) {
  ...
  for (let key in keyframes) {
    ...
    if (definition) {
      initialKeyframe = isNumber(value) ? definition.toDefaultUnit?.(value) : typeof value === "string" || typeof value === "number" ? value : void 0;
...
const STATE_TYPES = ["initial", "animate", "inView", "hover", "press", "exit"];
const mountedStates = /* @__PURE__ */ new WeakMap();
const motionStatesIdElements = /* @__PURE__ */ new Map();
function getStates() {
  return {
    mountedStates,
...
class MotionState {
  ...
  initContext() {
    for (const name of STATE_TYPES) {
      this.context[name] = typeof this.options[name] === "string" ? this.options[name] : this.parent?.context[name];
    }
  }
  initTarget(initialVariantSource) {
    this.baseTarget = resolveVariant(
      this.options[initialVariantSource] || this.context[initialVariantSource],
  ...
  mount(element) {
    ...
    visualElement?.update(this.options, this.parent?.context);
  ...
  *animateUpdates() {
    ...
    allTargetKeys.forEach((key) => {
      ...
      if (hasChanged(prevTarget[key], this.target[key])) {
        if (this.element)
          this.baseTarget[key] ??= style.get(this.element, key);
    ...
    yield;
    const animations = animationFactories.map((factory) => factory()).filter((animation) => animation !== void 0);
...
const transformProps$2 = new Set(transformPropOrder$1);
...
const numTransforms = transformPropOrder$1.length;
...
function generateHTMLStyles(latest, {
  style = {},
  vars = {},
  transform = {},
  transformOrigin = {},
...
function mergeStyles(el, newStyles) {
  if (!el || !newStyles)
    return;
  const style = {};
...
const transformProps$1 = new Set(transformPropOrder);
...
function criticallyDampedSpring(target) {
  return {
    type: "spring",
    stiffness: 550,
    damping: target === 0 ? 2 * Math.sqrt(550) : 30,
...
function getDefaultTransition(valueKey, value) {
  const keys = valueKey.split(",");
  if (value?.times && value?.times.length > 2) {
    return keyframesTransition;
  } else if (keys.some((key) => transformProps$1.has(key))) {
    if (keys.some((key) => key.startsWith("scale"))) {
      return value?.times && value?.times.length ? criticallyDampedSpring(value?.times[1]) : underDampedSpring;
    }
...
const [provideMotion, useMotion, motionKey] = createContext("MotionContext", null);
const [provideMotionPresence, useMotionPresence, presenceKey] = createContext("MotionPresenceContext", {
  initial: ref(false)
});
...
function convertFalseToUndefined(props) {
  return Object.entries(props).reduce((acc, [key, value]) => {
    return {
      ...acc,
      [key]: value === false ? void 0 : value
    };
  }, {});
}
...
function defaultTransition(_props) {
  const { isDefaultTransition = false } = _props;
  const props = transformProps(_props);
  if (isDefaultTransition) {
    const defaultTransitionValue = props.animate ? getDefaultTransition(
...
function useMotionHelper(_props, currentElement, sfc = true) {
  ...
  onUpdated(() => {
    const props2 = defaultTransition(_props);
    ...
    state.update({
      ...props2,
      initial: presenceInitial.value === false ? presenceInitial.value : props2.initial === true ? void 0 : props2.initial
    });
  });
  function getSVGProps() {
    if (!state.isMounted() && isSvgElementName(props.as))
      return state.getTarget();
  }
  function getStyle() {
    if (isSvgElementName(props.as))
      return props.style;
...
const _sfc_main$1 = /* @__PURE__ */ defineComponent({
  ...
  props: {
    ...
    variants: {},
    transition: {},
    isDefaultTransition: { type: Boolean, default: false },
    onMotionStart: {},
    onMotionComplete: {},
    onHoverStart: {},
    onHoverEnd: {},
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _cache) => {
      return openBlock(), createBlock(resolveDynamicComponent(_ctx.multiple ? TransitionGroup : Transition), mergeProps({
  ...
  const animate = (el, props) => {
    ...
    if (data.length > 0) {
      data.forEach((el2, index) => {
        const transition = props.transition ? typeof props.transition === "function" ? props.transition(index) : props.transition : {};
        if (!el2?.tagName) {
        ...
        if (svgProps) {
          Object.entries(svgProps).forEach(([key2, value]) => {
...
function resolveElement(elements, parentEl) {
  if (typeof elements === "string") {
    let root = document;
    if (parentEl) {
      invariant(
...
const motionPlugin = {
  install(app) {
    const MotionPresenceProps = reactive({
      mode: "sync",
      initial: true,
      multiple: false
    });
    ...
    app.provide(presenceKey, MotionPresenceProps);
  }
};
...
export { _sfc_main$1 as Motion, _sfc_main as MotionPresence, buildTransform, components, generateHTMLStyles, getDefaultTransition, mergeStyles, motionKey, motionPlugin, presenceKey, provideMotion, provideMotionPresence, useAnimate, useAnimations, useMotion, useMotionPresence, utilities };
